...
import { ExternalLink, TrendingUp, Award, CalendarDays, Clock, Trophy } from "lucide-react"
...
interface EntityHoverCardProps {
  ...
  logo: string
  website?: string
  type: "player" | "team" | "platform" | "organization"
  founded?: string
  description: string
...
export default function EntityHoverCard({
  ...
  const getTypeIcon = () => {
    switch (type) {
      case "player":
        return <Award size={12} className="mr-1.5 text-red-500 dark:text-red-400 flex-shrink-0" />
      case "team":
        return <Trophy size={12} className="mr-1.5 text-red-500 dark:text-red-400 flex-shrink-0" />
      case "platform":
        return <TrendingUp size={12} className="mr-1.5 text-red-500 dark:text-red-400 flex-shrink-0" />
      case "organization":
        return <Clock size={12} className="mr-1.5 text-red-500 dark:text-red-400 flex-shrink-0" />
      default:
        return null
    }
  }
            ...
            <div className="bg-gray-50 dark:bg-gray-900 p-4 border-b border-gray-200 dark:border-gray-700/80">
              <div className="flex items-center gap-3">
                <span className="relative w-12 h-12 rounded-md overflow-hidden border border-gray-200 dark:border-gray-700 flex-shrink-0 shadow-sm">
                  <Image src={logo || "/placeholder.png"} alt={`${name} logo`} fill className="object-contain p-1" />
                </span>
                <div>
                  ...
                  {founded && (
                    <span className="text-xs text-gray-500 dark:text-gray-400 flex items-center mt-0.5">
                      <CalendarDays size={10} className="mr-1" /> Founded: {founded}
                    </span>
                  )}
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                  {stats.map((stat, index) => (
                    <div key={index} className="flex items-center text-gray-600 dark:text-gray-400">
                      <div><strong>{stat.label}:</strong> {stat.value}</div>
                    </div>
                  ))}
                </div>
              )}
              ...
              {achievements.length > 0 && (
                <div>
                  <div className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5 flex items-center">
                    <Trophy size={12} className="mr-1.5 text-red-500 dark:text-red-400" /> Achievements:
                  </div>
  ...
  return (
    <>
      <span 
        ref={triggerRef}
        className="cursor-pointer hover:text-red-500 dark:hover:text-red-400 transition-colors duration-300"
        onMouseEnter={handleMouseEnter}
